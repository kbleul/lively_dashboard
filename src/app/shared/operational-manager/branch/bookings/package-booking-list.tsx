...
export default function PackageBookingList({
  ...
  const bookingsList = useFetchData(
    ...
    `${process.env.NEXT_PUBLIC_SERVICE_BACKEND_URL}operation-manager/upcoming-bookings/${branchId}?page=${currentPage}&per_page=${pageSize}`,
    headers
  );
  ...
  const onApproveItem = async (id: string) => {
    try {
      await postMutation.mutateAsync({
        url: `${process.env.NEXT_PUBLIC_SERVICE_BACKEND_URL}operation-manager/complete-booking/${id}`,
        method: "POST",
        headers,
        body: {},
        onSuccess: () => {
  ...
  const onRejectItem = async (id: string) => {
    try {
      await postMutation.mutateAsync({
        url: `${process.env.NEXT_PUBLIC_SERVICE_BACKEND_URL}operation-manager/cancel-booking/${id}`,
        method: "POST",
        headers,
        body: {},
        onSuccess: () => {
