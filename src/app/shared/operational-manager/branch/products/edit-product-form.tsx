...
const EditdProductForm = ({
  ...
  const productData = useFetchData(
    ...
    `${process.env.NEXT_PUBLIC_SERVICE_BACKEND_URL}operation-manager/show-branch-products/${productId}`,
    headers
  );
  ...
  const editBranchProduct = async (values: EditBranchProductype) => {
    try {
      await postMutation.mutateAsync({
        url: `${process.env.NEXT_PUBLIC_SERVICE_BACKEND_URL}operation-manager/branch-products/${productId}`,
        method: "POST",
        headers,
        body: {
          price: values.price,
