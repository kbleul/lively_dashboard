...
const CheckMemberExits = ({
  ...
  const searchMemberSubmitHandler = async (values: SearchMemberType) => {
    try {
      await postMutation.mutateAsync({
        url: `${process.env.NEXT_PUBLIC_SERVICE_BACKEND_URL}store-owner/find-user?phone=251${values.phone}`,
        method: "GET",
        headers,
        body: {
          ...values,
  ...
  const addPackagesSubmitHandler = async (values: AddPackagesType) => {
    try {
      await postMutation.mutateAsync({
        url: `${process.env.NEXT_PUBLIC_SERVICE_BACKEND_URL}store-owner/add-existing-member/${member?.id}`,
        method: "POST",
        headers,
        body: {
          ...values,
        ...
        onSuccess: (res) => {
          ...
          router.push(
            routes.storeOwner.branch["list-members"](placeId, branchId)
          );
        },
        onError: (err) => {
          toast.error(err?.response?.data?.data);
