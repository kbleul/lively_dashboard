...
const ClaimedDetails = ({ discountId }: { discountId: string }) => {
  ...
  const claimData = useFetchData(
    ...
    `${process.env.NEXT_PUBLIC_SERVICE_BACKEND_URL}branch-manager/claimed-product-discount-detail/` +
      discountId,
    headers
  );
  ...
  const applyClaim = async (id: string) => {
    try {
      await postMutation.mutateAsync({
        url:
          `${process.env.NEXT_PUBLIC_SERVICE_BACKEND_URL}branch-manager/apply-claimed-product-discounts/` +
          id,
        method: "POST",
        headers,
        body: {},
