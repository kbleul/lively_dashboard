...
const ReorderPackages = () => {
  ...
  const storeData = useFetchData(
    [queryKeys.getAllPackageCategories],
  ...
  const redorderPackageCategories = async () => {
    try {
      ...
      await postMutation.mutateAsync({
        url: `${process.env.NEXT_PUBLIC_SERVICE_BACKEND_URL}branch-manager/packages-category-order`,
        method: "POST",
        headers,
        body: {
        ...
        onSuccess: () => {
          queryClient.invalidateQueries({
            queryKey: [queryKeys.getAllPackageCategories],
          });
          ...
          queryClient.setQueryData([queryKeys.getAllPackageCategories], null);
