...
const AddQuestionForm = ({ categoryId }: { categoryId: string }) => {
  ...
  const numberOfQuestions = useSearchParams().get("number_of_questions");
  ...
  const initialValues: assessmentQuestionsType = {
    questions: Array.from(
      { length: numberOfQuestions ? parseInt(numberOfQuestions) : 0 },
      () => ({
        question_text: "",
        options: [],
      })
  ...
  return (
    <article>
      {!numberOfQuestions && (
        <p className="text-lg w-full text-center mt-24">
          Qusetion number not provided
        </p>
      )}
      {numberOfQuestions && (
        <Formik
